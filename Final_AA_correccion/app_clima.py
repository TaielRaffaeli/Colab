...
pipeline_entrenado2 = joblib.load('clima_pipeline_regre.joblib')
...
Humidity9am = st.slider('Humidity9am', 1.0, 100.0, 3.0)
...
if st.button('Predecir'):
    ...
    prediccion2 = pipeline_entrenado2.predict(data_para_predecir)
    
    umbral = 0.77 #segun curva ROC

    if prediccion > umbral:
        st.write('Predicción: Si llueve')
        st.write(f'Con una certeza del {int(prediccion*100)}%')  # Convertir la parte decimal a entero
        st.write('*--------------*')    
        st.write(f'Cantidad de lluvia: {int(prediccion2)} mm')  # Convertir la parte decimal a entero
    else:
        st.write('Predicción: No llueve')
        st.write(f'Con una certeza del {int(prediccion*100)}%') 
